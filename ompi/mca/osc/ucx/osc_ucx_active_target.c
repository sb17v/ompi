...
int ompi_osc_ucx_fence(int mpi_assert, struct ompi_win_t *win) {
    ...
    int i, local_rank, target_rank, ret = OMPI_SUCCESS, status = -1;
    ...
    if (!(mpi_assert & MPI_MODE_NOPRECEDE)) {
        ...
        /* TODO: We need no flush every target but it's pretty heavy*/
        /* THINK: Alternate impl may use two barrier*/
        /* Flush the all connected dpu for hc channel and mpi1sdd channel */
...
int ompi_osc_ucx_complete(struct ompi_win_t *win) {
    ompi_osc_ucx_module_t *module = (ompi_osc_ucx_module_t*) win->w_osc_module;
    int i, size, local_rank, target_rank;
